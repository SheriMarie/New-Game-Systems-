...
{
    // RAY - A ray is an infinite line (in 1 direction) starting (extending) from a starting originand going in some... specified direction

    //RAYCAST/RAYCASTING - Instructions for sending the ray, from an origin point, in the direction, for a search length, against all/specified colliders in the scene, on the layer of search 

    //RAYCAST HIT - the structure of data used to get information back from the ray collision with a collider 

    //LAYER/LAYERMASK - Layer mask is the chosen layer or layers we are searching/interacting with 
